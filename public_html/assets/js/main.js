...
(function($) {
	...
	$(document).ready( function() {
		...
		if( $('.map-canvas').length > 0) {
			...
			var address = 'Rua Goitacaz, 304, Vila Assunção, Porto Alegre, RS, Brasil';
			var contentString = '<div class="map-detail"><strong>Nosso endereço:</strong><p>' + address + '</p></div>';
			...
			geocoder.geocode({'address': address }, function(results, status) {
				if(status == google.maps.GeocoderStatus.OK) { 
					...
					jQuery('.map-canvas').gmap().bind('init', function(ev, map) {
						...
						jQuery('.map-canvas').gmap('option', 'zoom', 15);
					});
				}else { alert('Google Maps não conseguiu encontrar o endereço. Status: ' + status); }
			});
